...
from ..models.staff import Company, CompanyStaff
...
class EventCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not companies:
            raise serializers.ValidationError("Staff does not belong to any company")

        if company_id not in companies:
            raise serializers.ValidationError("You do not belong to this company")

        company = CompanyStaff.objects.get(staff=staff, company_id=company_id).company
        ...
        if company_id and category:
            ...
            if not exists:
                raise serializers.ValidationError(
                    {"category": "This category does not belong to the selected company."}
                )
...
class EventImageUploadSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        return EventImage.objects.create(event=event, **validated_data)
